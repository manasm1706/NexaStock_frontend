...
import { Sparkles, ArrowRight, Brain, GitBranch, Wand2, Send, Zap, Loader2 } from "lucide-react";
...
function AIPage() {
  ...
  const insights = useMemo(() => {
    ...
    return aiInsights.recommendations.map((rec: string, index: number) => {
      const isReorder = rec.toLowerCase().includes("reorder") || rec.toLowerCase().includes("replenishment");
      return {
        tag: isReorder ? "Forecast" : "Redistribute",
        title: rec,
        body: isReorder 
          ? "Reorder recommended quantity based on seasonality forecasts and current safety stock level."
          : "Balance network quantities to satisfy upcoming demand spikes.",
        ...
        <GlassCard className="p-6">
          ...
          <div className="relative">
            ...
            <div className="mt-3 flex items-center gap-2 rounded-xl border border-white/10 bg-background/40 px-4 py-3">
              ...
              />
              <Button variant="premiumGradient" size="sm" className="h-8 px-3 text-xs">
            ...
            <div className="flex flex-wrap gap-2 mt-3">
              {[
                ...
                <button key={q} className="text-xs px-3 py-1.5 rounded-full border border-white/10 bg-white/3 text-muted-foreground hover:text-foreground">
                  {q}
                </button>
              ))}
            </div>
      ...
      <div className="grid lg:grid-cols-3 gap-4">
        <div className="lg:col-span-2 space-y-4">
          <div className="flex items-center justify-between">
            <SectionTitle>Active insights</SectionTitle>
            <span className="text-xs text-muted-foreground">Updated 1 min ago · {insights.length} active suggestions</span>
          </div>
            ...
            {insights.map((it: any, i: number) => (
              ...
              >
                <GlassCard className="p-5">
                  ...
                  <div className="mt-2 font-medium leading-snug text-foreground">{it.title}</div>
                  <div className="mt-2 text-xs text-muted-foreground">{it.body}</div>
                  <div className="mt-4 h-1 rounded-full bg-white/5 overflow-hidden">
                    <div className="h-full bg-linear-to-r from-primary to-accent" style={{ width: `${it.confidence}%` }} />
                  </div>
        ...
        <div className="space-y-4">
          <SectionTitle>Autonomous agents</SectionTitle>
          ...
          <GlassCard className="p-5">
            ...
            <div className="mt-1 font-display text-base text-foreground">NexaStock-Forecast v3.1</div>
            <div className="mt-3 grid grid-cols-3 gap-2 text-center">
              {[{ l: "MAPE", v: "4.8%" }, { l: "Coverage", v: "96%" }, { l: "Latency", v: "180ms" }].map((m) => (
                <div key={m.l} className="rounded-xl border border-white/10 bg-white/2 py-2">
                  <div className="text-[10px] uppercase tracking-widest text-muted-foreground">{m.l}</div>
                  <div className="text-sm font-medium mt-0.5 text-foreground">{m.v}</div>
                </div>
              ))}
            </div>
          </GlassCard>
